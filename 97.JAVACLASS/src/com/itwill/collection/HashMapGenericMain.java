...
public class HashMapGenericMain {
	...
	public static void main(String[] args) {
		...
		Car removeCar = carMap.remove("1111");
		
		removeCar = carMap.remove("0000");
		if(removeCar == null){
			System.out.println("0000 번호 존재 안함");
		}
		System.out.println("------------------- remove -------------------");
		if(carMap.containsKey("4444")){
			removeCar = carMap.remove("4444");
			System.out.println(">> map : " + carMap);
		}
		...
		Iterator<String> keyset = keySet.iterator();
		while (keyset.hasNext()) {
			String tempKey = keyset.next();
